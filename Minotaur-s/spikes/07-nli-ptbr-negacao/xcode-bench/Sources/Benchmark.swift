...
/// SPIKE 7 — harness do Spike 2c, reaproveitado, com UMA extensão: `token_type_ids`.
///
/// Os candidatos deste spike são BERTimbau (BERT), e BERT separa premissa de
/// hipótese por segmento. Sem essa terceira entrada o modelo roda e devolve
/// logits errados **sem erro nenhum** — o pior modo de falha possível para um
/// gate cujo propósito é justamente distinguir "executou" de "executou correto".
/// A comparação com `pytorch_logits` pegaria a divergência, mas o gate ficaria
/// medindo a interface errada em vez do modelo.
///
...
struct ModelSpec: Decodable {
    ...
    /// `nil` para modelos sem segmentos (XLM-R/RoBERTa); presente para BERT.
...
enum Benchmark {
    ...
    static func makeInput(ids: [Int], mask: [Int], types: [Int]?) throws -> MLDictionaryFeatureProvider {
        func array(_ values: [Int]) throws -> MLMultiArray {
            let arr = try MLMultiArray(shape: [1, NSNumber(value: values.count)], dataType: .int32)
            for i in values.indices { arr[i] = NSNumber(value: Int32(values[i])) }
            return arr
        }

        var features: [String: MLFeatureValue] = [
            "input_ids": MLFeatureValue(multiArray: try array(ids)),
            "attention_mask": MLFeatureValue(multiArray: try array(mask)),
        ]
        if let types {
    ...
    static func run(spec: ModelSpec, units: MLComputeUnits, unitsLabel: String,
        ...
        do { input = try makeInput(ids: ids, mask: mask, types: spec.token_type_ids) }
